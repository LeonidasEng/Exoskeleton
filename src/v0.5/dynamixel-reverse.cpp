...
* This scripts is being used to explore: Reverse operation on new Dynamixel motor
...
int ID1_pos2_center = basePosition - 4096; // One rotation in the negative direction
int ID1_pos3_center = basePosition - 2 * 4096; // Two rotations in the negative direction
...
boolean setSpeed(Dynamixel2Arduino &dxl, uint8_t DXL_1, float speedPct) 
{
    double maxDynamixelSpeed = 1023 * 0.229; //RPM
    uint32_t newSpeedRpm = speedPct * maxDynamixelSpeed / 100;
    uint32_t writeTimeout = 100; // ms
    return dxl.writeControlTableItem(PROFILE_VELOCITY, DXL_1, newSpeedRpm, writeTimeout);
}
...
void setup() {
  ...
  if (!setSpeed(dxl, DXL_1, speedPct)) 
  {
    Serial.println("Failed to set speed");
  }
  int present_position = dxl.getPresentPosition(DXL_1);
    // if (presentposition < 2048) {
    //   dxl.setGoalPosition(DXL_1, 2048);
  ...
  dxl.torqueOff(DXL_1);
  dxl.setOperatingMode(DXL_1, OP_EXTENDED_POSITION);
  dxl.torqueOn(DXL_1);
...
void loop() {
  ...
  dxl.setGoalPosition(DXL_1, basePosition);  // set to base position
  //Serial.println("0");
