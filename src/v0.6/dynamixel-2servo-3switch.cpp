...
void setup() {
  ...
  for (int i = 0; i < sizeof(DXL_IDs) / sizeof(DXL_IDs[0]); i++) {
    if (dxl.ping(DXL_IDs[i])) {
      ...
      DEBUG_SERIAL.println("Failed to connect to Dynamixel motor with ID " + String(DXL_IDs[i]) + ".");
    }
  }
  ...
  DEBUG_SERIAL.println("Operating Mode changed to: Position Mode");
...
void SerialMon()
{
  ...
  if (millis() - lastMillis >= interval)
  {
    ...
    DEBUG_SERIAL.print(dxl.getPresentPosition(DXL_IDs[0], UNIT_DEGREE));
    DEBUG_SERIAL.print(" | Motor 2 (ID 3) Position 2: ");
    DEBUG_SERIAL.println(dxl.getPresentPosition(DXL_IDs[1], UNIT_DEGREE));
